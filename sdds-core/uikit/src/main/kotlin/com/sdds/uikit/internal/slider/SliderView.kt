...
internal class SliderView @JvmOverloads constructor(
    ...
    private var _limitLabelAlignment: Int = LIMIT_LABEL_ALIGNMENT_END
    ...
    private var _maxProgress: Float = 1f
    ...
    private var _valueMode: Int = VALUE_MODE_INTERACTION
    ...
    var slideDirection: Int
        ...
        set(value) {
            ...
            sliderDrawable.updateThumbOffset(trackLength)
        }
        ...
        set(value) {
            if (_limitLabelAlignment != value) {
                ...
                if (limitLabelEnabled) {
                    if (oldValue == LIMIT_LABEL_ALIGNMENT_CENTER ||
                        _limitLabelAlignment == LIMIT_LABEL_ALIGNMENT_CENTER
                    ) {
                        requestLayout()
                    } else {
                        updateDrawablesBounds()
        ...
        set(value) {
            if (_valueMode != value) {
                ...
                if (tooltip?.isShowing == true) {
                    if (valueMode == VALUE_MODE_NONE) tooltip?.dismiss()
                }
            }
        }
     ...
     */
    fun getCurrentValue() = _minProgress + (_maxProgress - _minProgress) * sliderDrawable.progress
    ...
    private fun offsetToTouch(touch: Float) {
        ...
        sliderDrawable.updateThumbOffset(trackLength, offset)
        _progressListener?.onProgressChanged(getCurrentValue())
        updateToolTipText()
        invalidate()
    }
    ...
    private fun followFinger(touch: Float) {
        ...
        if (tooltip?.isShowing == false && valueMode == VALUE_MODE_INTERACTION) showValue()
    }
    ...
    private fun updateSlideFromDrag(event: MotionEvent) {
        ...
        sliderDrawable.updateThumbOffset(trackLength, position.toInt())
    ...
    private fun updateBoundsWhenHorizontal() {
        ...
        trackLength = sliderDrawable.bounds.width()
    }
    ...
    private fun updateBoundsWhenVertical() {
        ...
        when (limitLabelAlignment) {
            LIMIT_LABEL_ALIGNMENT_CENTER -> {
                ...
                startLabel.setBounds(halfW - startW / 2, height - pB - startH, halfW + startW / 2, height - pB)
                endLabel.setBounds(halfW - endW / 2, pT, halfW + endW / 2, pT + endH)
            }
        ...
        trackLength = sliderDrawable.bounds.height()
    }
    ...
    private fun getSliderThicknessWithLabel(): Int {
        ...
        return if (limitLabelAlignment == LIMIT_LABEL_ALIGNMENT_CENTER) {
            maxOf(sliderThickness, limitLabelThickness)
        } else {
            lineThickness + extra + limitLabelThickness
        }
    }
    ...
    private fun getLimitLabelsWidth(): Int {
        ...
        val width = if (isHorizontal) {
            if (limitLabelAlignment == LIMIT_LABEL_ALIGNMENT_CENTER) {
