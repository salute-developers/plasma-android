...
internal class SliderDrawable(
    ...
    private var mThumbOffset: Int = 0
    ...
    // mThumbForStrokeDrawable за счет меньшего диаметра создает бордер, его размеры не учавствуют расчетах
    ...
    /**
     * Устанавливает прогресс [progress], принимает значения от 0f до 1f
     */
    fun setProgress(progress: Float) {
        val clamped = progress.coerceIn(0f, 1f)
        val trackLength = if (orientation == HORIZONTAL) bounds.width() else bounds.height()
        val offset = (trackLength * clamped).toInt() - thumbSize / 2
        updateThumbOffset(trackLength, offset)
    }
     ...
     */
    fun updateThumbOffset(trackLength: Int, position: Int) {
        val track = (trackLength - _thumbSize).coerceAtLeast(0)
        mThumbOffset = position.coerceIn(0, track)
        ...
        progress = if (track > 0) mThumbOffset.toFloat() / track else 0f
        invalidateSelf()
    }
     ...
     */
    fun updateThumbOffset(trackLength: Int) {
        val track = (trackLength - _thumbSize).coerceAtLeast(0)
        val position = (progress * track).toInt()
        updateThumbOffset(trackLength, position)
    }
    ...
    fun calculateDragOffset(touch: Float): Float {
        val touchOffset = touchToOffset(touch).toFloat()
    ...
    fun touchToOffset(touch: Float): Int {
        return when (orientation) {
            HORIZONTAL -> {
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    (bounds.right - touch).toInt().coerceIn(0, bounds.width())
                } else {
                    (touch - bounds.left).toInt().coerceIn(0, bounds.width())
                }
            }

            VERTICAL -> {
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    (touch - bounds.top).toInt().coerceIn(0, bounds.height())
                } else {
                    (bounds.bottom - touch).toInt().coerceIn(0, bounds.height())
                }
            }
    ...
    private fun updateIndicatorBounds() {
        ...
        when (orientation) {
            VERTICAL -> {
                ...
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    top = bounds.top
                    bottom = top + mThumbOffset + extraOffset
                } else {
                    bottom = bounds.bottom
                    top = bottom - mThumbOffset - extraOffset
                }
            }
                ...
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    right = bounds.right
                    left = right - mThumbOffset - extraOffset
                } else {
                    left = bounds.left
                    right = left + mThumbOffset + extraOffset
                }
            }
        }
    ...
    private fun updateThumbBounds() {
        ...
        when (orientation) {
            VERTICAL -> {
                ...
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    top = bounds.top + mThumbOffset
                    bottom = top + thumbSize
                } else {
                    bottom = bounds.bottom - mThumbOffset
                ...
                bottom = top + thumbSize
                if (sliderDirection == SLIDE_DIRECTION_REVERSED) {
                    right = bounds.right - mThumbOffset
                    left = right - thumbSize
                } else {
                    left = bounds.left + mThumbOffset
                    right = left + thumbSize
                }
            }
        }
