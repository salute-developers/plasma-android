...
fun Text(
    ...
    val brush = remember(textColor) { SolidColor(textColor) }
    BaseText(
        source = source,
        modifier = modifier,
        style = style,
...
fun Text(
    ...
    Text(
        ...
        softWrap = softWrap,
        maxLines = maxLines,
        brush = brush ?: color?.let { BrushProducer { SolidColor(color.invoke()) } },
    )
}
...
fun ProvideTextStyle(value: TextStyle, color: ColorProducer, content: @Composable () -> Unit) {
    ...
    val fallbackBrushProducer = remember(color) { BrushProducer { SolidColor(color.invoke()) } }
