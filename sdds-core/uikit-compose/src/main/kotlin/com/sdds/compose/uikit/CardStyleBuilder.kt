...
/**
 * CompositionLocal с [CardStyle] для компонента [CardSolid]
 */
val LocalCardStyle = compositionLocalOf { CardStyle.cardSolidBuilder().style() }

/**
 * Возвращает экземпляр [CardStyleBuilder] для Solid
 */
fun CardStyle.Companion.cardSolidBuilder(receiver: Any? = null): CardStyleBuilder =
    DefaultCardStyleBuilderImpl(receiver)

/**
 * Возвращает экземпляр [CardStyleBuilder] для Clear
 */
fun CardStyle.Companion.cardClearBuilder(receiver: Any? = null): CardStyleBuilder =
    DefaultCardStyleBuilderImpl(receiver)
