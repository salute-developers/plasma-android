...
import com.sdds.compose.uikit.motion.components.tabs.TabsMotionStyle
import com.sdds.compose.uikit.motion.components.tabs.rememberTabsMotion
...
@Composable
@Deprecated(
    "Use Tabs with selectedTabIndexProvider",
    replaceWith = ReplaceWith(
        "Tabs(selectedTabIndexProvider = { })",
    ),
)
fun Tabs(
 ...
 * В компоненте доступно отображение максимум 10 табов.
 *
 * @param selectedTabIndexProvider индекс выбранного таба
 * @param modifier модификатор
 * @param style стиль компонента [TabsStyle]
 * @param onDisclosureTabClicked обработчик нажатия на таб внутри disclosure.
 * В параметре лямбды доступен индекс нажатого таба.
 * @param clip режимразмещения ограничения размеров контейнера с табами [TabsClip]
 * @param stretch занимают ли табы доступное пространство
 * @param enabled включает/выключает компонент
...
fun Tabs(
    ...
    dividerEnabled: Boolean = style.dividerEnabled,
    orientation: TabsOrientation = style.orientation,
    motion: Motion<TabsMotionStyle> = rememberTabsMotion(),
    tabs: TabScope.() -> Unit,
) {
    BaseTabs(
