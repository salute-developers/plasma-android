...
internal class UniversalComposeVariationGenerator(
    private val componentMeta: ComposeComponentMeta,
    ...
    private fun propsToBuilderCalls(
        ...
        paramOverloads.forEach { overloads ->
            ...
            val call: String = when (meta) {
                is ComposeColorPropertyMeta -> mappers.colorMapper.map(meta, value as Color, variationId)
                is ComposeDimensionPropertyMeta -> mappers.dimensionMapper.map(meta, value as Dimension, variationId)
                is ComposeTypographyPropertyMeta -> mappers.typographyMapper.map(meta, value as Typography, variationId)
                is ComposeShapePropertyMeta -> mappers.shapeMapper.map(meta, value as Shape, variationId)
                is ComposeShadowPropertyMeta -> mappers.shadowMapper.map(meta, value as Shadow, variationId)
                is ComposeBooleanPropertyMeta -> mappers.booleanMapper.map(meta, value as Value, variationId)
                is ComposeIntegerPropertyMeta -> mappers.integerMapper.map(meta, value as Value, variationId)
                is ComposeFloatPropertyMeta -> mappers.floatMapper.map(meta, value as FloatValue, variationId)
                is ComposeIconPropertyMeta -> mappers.iconMapper.map(meta, value as Value, variationId)
                is ComposeComponentPropertyMeta ->
                    mappers.componentStyleMapper.map(meta, value as ComponentStyle<*>, variationId)
                is ComposeValuePropertyMeta -> mappers.valueMapper.map(meta, value as Value, variationId)
                is ComposeUnknownPropertyMeta -> return@forEach
            }
...
internal fun ComposePropertyMeta.overloadPriority(): Int = when {
    "StatefulValue" in paramQualifiedType -> 2
    "InteractiveColor" in paramQualifiedType -> 1
    else -> 0
}

internal fun List<ComposePropertyMeta>.selectOverload(value: Stateful<*, *>): ComposePropertyMeta {
    val colorOverloads = filterIsInstance<ComposeColorPropertyMeta>()
...
private fun ComposeColorPropertyMeta.colorOverloadPriority(): Int = when {
