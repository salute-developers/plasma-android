...
аннотации и методы билдера, формирует `uikit-compose-api-meta.json`, а Theme Builder
...
7. Убедитесь, что каждый параметр имеет поддерживаемый тип и что иконки помечены
   `@DrawableRes`.
8. Ничего регистрировать не нужно: компонент конфига сводится с метой по имени.

### Как компонент находит свою мету

Реестра компонентов нет. Ключ компонента в конфиге темы (`basic-button`) переводится
в имя компонента в мете (`BasicButton`), и по нему берутся обе меты: compose — из
`@ApiInfo` uikit-compose, View — из разметки `sdds:api_*` в ресурсах uikit
(см. [руководство для View](VIEW_STYLE_GENERATOR.md)).

Компонент, у которого нет меты для цели, для этой цели не генерируется: так
компоненты, существующие только в Compose (`tab-bar`, `bottom-sheet`, `splitter`, …),
корректно отсутствуют во View-темах. Единственное историческое расхождение ключа и
имени — `bottom-sheet` → `ModalBottomSheet` — покрыто таблицей алиасов в
`UniversalComponentGenerator`.

## Проверка

Сначала проверьте генерацию KSP-метаданных и упаковку UIKit Compose:

```shell
./gradlew :sdds-core:uikit-compose:assemble
```
...
`uikit-compose-api-meta.json` в UIKit Compose-зависимости Theme Builder.
