 ...
 *
 * Мультиплатформенная реализация: размер контейнера берётся из [LocalWindowInfo],
 * что доступно на всех таргетах Compose Multiplatform, без обращения к
 * платформенным API окружения Android.
 */
@Composable
fun collectWindowSizeInfoAsState(): State<WindowSizeInfo> {
    ...
    return remember(
        ...
        mediumHeightBreakPoint,
        expandedWidthBreakPoint,
        expandedHeightBreakPoint,
    ) {
        derivedStateOf {
            val size: IntSize = windowInfo.containerSize
            val widthClass = when {
