 ...
 * common state controller и системой Scaffold для обеспечения работы панели управления свойствами.
 *
...
abstract class ComposeBaseStory<State : UiState, S : Style>(
     ...
     *
     * Создает [ComponentScaffold] с state controller, который обеспечивает:
    ...
    internal fun Story() {
        ...
        DisposableEffect(stateController) {
            stateOwner = stateController
            onDispose {
                if (stateOwner === stateController) {
                    stateOwner = null
                }
            }
        }
