...
private val AvatarStyleBuilder.invariantProps: AvatarStyleBuilder
    ...
    get() = this
        ...
        .colors {
            backgroundColor(
                SddsSandboxTheme.gradients.surfaceDefaultAccentGradient.asLayered(0.2f).asStatefulValue(),
            )
            textColor(
                SddsSandboxTheme.gradients.textDefaultAccentGradient.asLayered().asStatefulValue(),
            )
        }
    ...
    get() = AvatarStyle.builder(this)
        ...
        .statusStyle(
            Indicator.L.modify {
                color {
                    backgroundColor(
                        SddsSandboxTheme.colors.surfaceOnLightSolidTertiary.asInteractive(
                            setOf(AvatarStatus.Active)
                                to SddsSandboxTheme.colors.surfaceDefaultPositive,
                        ),
                    )
                }
            }
