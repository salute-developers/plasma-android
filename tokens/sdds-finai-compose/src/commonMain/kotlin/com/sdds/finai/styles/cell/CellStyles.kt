// AUTO-GENERATED. DO NOT MODIFY this file.
@file:Suppress(
    "UndocumentedPublicClass",
    "UndocumentedPublicProperty",
    "ktlint:standard:max-line-length",
)

package com.sdds.finai.styles.cell

import androidx.compose.runtime.Composable
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.unit.dp
import com.sdds.compose.uikit.CellStyle
import com.sdds.compose.uikit.CellStyleBuilder
import com.sdds.compose.uikit.imageVectorSource
import com.sdds.compose.uikit.interactions.asStatefulValue
import com.sdds.compose.uikit.style.BuilderWrapper
import com.sdds.compose.uikit.style.style
import com.sdds.compose.uikit.style.wrap
import com.sdds.finai.styles.avatar.Avatar
import com.sdds.finai.styles.avatar.L
import com.sdds.finai.styles.avatar.M
import com.sdds.finai.styles.avatar.S
import com.sdds.finai.styles.checkbox.CheckBox
import com.sdds.finai.styles.checkbox.Default
import com.sdds.finai.styles.checkbox.M
import com.sdds.finai.styles.checkbox.S
import com.sdds.finai.styles.iconbutton.Clear
import com.sdds.finai.styles.iconbutton.IconButton
import com.sdds.finai.styles.iconbutton.S
import com.sdds.finai.styles.iconbutton.Xs
import com.sdds.finai.styles.radiobox.M
import com.sdds.finai.styles.radiobox.RadioBox
import com.sdds.finai.styles.radiobox.S
import com.sdds.finai.styles.switcher.L
import com.sdds.finai.styles.switcher.M
import com.sdds.finai.styles.switcher.S
import com.sdds.finai.styles.switcher.Switch
import com.sdds.finai.theme.SddsFinAiTheme
import com.sdds.icons.compose.DisclosureRightOutline16
import com.sdds.icons.compose.DisclosureRightOutline24
import com.sdds.icons.compose.SddsIcons
import kotlin.Suppress
import kotlin.jvm.JvmInline
import kotlin.jvm.JvmName

/**
 * Базовый интерфейс для всех оберток этого стиля
 */
public interface WrapperCell : BuilderWrapper<CellStyle, CellStyleBuilder>

/**
 * Обертка для вариации L
 */
@JvmInline
public value class WrapperCellL(
    public override val builder: CellStyleBuilder,
) : WrapperCell

/**
 * Обертка для вариации M
 */
@JvmInline
public value class WrapperCellM(
    public override val builder: CellStyleBuilder,
) : WrapperCell

/**
 * Обертка для вариации S
 */
@JvmInline
public value class WrapperCellS(
    public override val builder: CellStyleBuilder,
) : WrapperCell

/**
 * Обертка для вариации Xs
 */
@JvmInline
public value class WrapperCellXs(
    public override val builder: CellStyleBuilder,
) : WrapperCell

private val CellStyleBuilder.invariantProps: CellStyleBuilder
    @Composable
    get() = this
        .colors {
            titleColor(SolidColor(SddsFinAiTheme.colors.textDefaultPrimary).asStatefulValue())
            labelColor(SolidColor(SddsFinAiTheme.colors.textDefaultSecondary).asStatefulValue())
            subtitleColor(SolidColor(SddsFinAiTheme.colors.textDefaultSecondary).asStatefulValue())
            disclosureTextColor(SolidColor(SddsFinAiTheme.colors.textDefaultSecondary).asStatefulValue())
            disclosureIconColor(SolidColor(SddsFinAiTheme.colors.textDefaultSecondary).asStatefulValue())
        }

public val Cell.L: WrapperCellL
    @Composable
    @JvmName("WrapperCellL")
    get() = CellStyle.builder(this)
        .invariantProps
        .labelStyle(SddsFinAiTheme.typography.bodyMNormal)
        .titleStyle(SddsFinAiTheme.typography.bodyLNormal)
        .subtitleStyle(SddsFinAiTheme.typography.bodyMNormal)
        .disclosureTextStyle(SddsFinAiTheme.typography.bodyMNormal)
        .disclosureIcon(imageVectorSource(SddsIcons.DisclosureRightOutline24))
        .avatarStyle(Avatar.L.style())
        .iconButtonStyle(IconButton.S.Clear.style())
        .checkBoxStyle(CheckBox.M.Default.style())
        .radioBoxStyle(RadioBox.M.style())
        .switchStyle(Switch.L.style())
        .dimensions {
            contentPaddingStart(8.0.dp)
            contentPaddingEnd(8.0.dp)
        }
        .wrap(::WrapperCellL)

public val Cell.M: WrapperCellM
    @Composable
    @JvmName("WrapperCellM")
    get() = CellStyle.builder(this)
        .invariantProps
        .labelStyle(SddsFinAiTheme.typography.bodySNormal)
        .titleStyle(SddsFinAiTheme.typography.bodyMNormal)
        .subtitleStyle(SddsFinAiTheme.typography.bodySNormal)
        .disclosureTextStyle(SddsFinAiTheme.typography.bodySNormal)
        .disclosureIcon(imageVectorSource(SddsIcons.DisclosureRightOutline24))
        .avatarStyle(Avatar.M.style())
        .iconButtonStyle(IconButton.S.Clear.style())
        .checkBoxStyle(CheckBox.M.Default.style())
        .radioBoxStyle(RadioBox.M.style())
        .switchStyle(Switch.M.style())
        .dimensions {
            contentPaddingStart(8.0.dp)
            contentPaddingEnd(8.0.dp)
        }
        .wrap(::WrapperCellM)

public val Cell.S: WrapperCellS
    @Composable
    @JvmName("WrapperCellS")
    get() = CellStyle.builder(this)
        .invariantProps
        .labelStyle(SddsFinAiTheme.typography.bodyXsNormal)
        .titleStyle(SddsFinAiTheme.typography.bodySNormal)
        .subtitleStyle(SddsFinAiTheme.typography.bodyXsNormal)
        .disclosureTextStyle(SddsFinAiTheme.typography.bodyXsNormal)
        .disclosureIcon(imageVectorSource(SddsIcons.DisclosureRightOutline16))
        .avatarStyle(Avatar.S.style())
        .iconButtonStyle(IconButton.S.Clear.style())
        .checkBoxStyle(CheckBox.S.Default.style())
        .radioBoxStyle(RadioBox.S.style())
        .switchStyle(Switch.S.style())
        .dimensions {
            contentPaddingStart(6.0.dp)
            contentPaddingEnd(6.0.dp)
        }
        .wrap(::WrapperCellS)

public val Cell.Xs: WrapperCellXs
    @Composable
    @JvmName("WrapperCellXs")
    get() = CellStyle.builder(this)
        .invariantProps
        .labelStyle(SddsFinAiTheme.typography.bodyXxsNormal)
        .titleStyle(SddsFinAiTheme.typography.bodyXsNormal)
        .subtitleStyle(SddsFinAiTheme.typography.bodyXxsNormal)
        .disclosureTextStyle(SddsFinAiTheme.typography.bodyXsNormal)
        .disclosureIcon(imageVectorSource(SddsIcons.DisclosureRightOutline16))
        .avatarStyle(Avatar.S.style())
        .iconButtonStyle(IconButton.Xs.Clear.style())
        .checkBoxStyle(CheckBox.S.Default.style())
        .radioBoxStyle(RadioBox.S.style())
        .switchStyle(Switch.S.style())
        .dimensions {
            contentPaddingStart(6.0.dp)
            contentPaddingEnd(6.0.dp)
        }
        .wrap(::WrapperCellXs)
