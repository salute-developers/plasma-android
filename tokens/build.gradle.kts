...
internal abstract class GenerateThemeModulesTask @Inject constructor() : DefaultTask() {
    ...
    private fun addGradlePropertiesFile(filePath: String, themeName: String, isCompose: Boolean) {
        ...
        File(filePath).apply {
            ...
            writeText(
                buildString {
                    appendLine("nexus.artifactId=${themeName.replacePointByDash()}-$framework-tokens")
                    appendLine("nexus.snapshot=false")
                    appendLine("nexus.description=${themeName.replacePointByDash()} token library for $framework framework")
