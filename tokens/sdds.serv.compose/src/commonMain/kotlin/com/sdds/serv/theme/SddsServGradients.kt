...
public class SddsServGradients(
    ...
    /**
     * Возвращает копию [SddsServGradients]. Предоставляет возможность переопределять цвета.
     */
    internal fun copyAttrs(overrideGradients: GradientAttrOverrideScope.() -> Unit = {}): SddsServGradients {
        val gradientOverrideScope = GradientAttrOverrideScope()
        overrideGradients.invoke(gradientOverrideScope)
        val overrideMap = gradientOverrideScope.overrideMap
        return SddsServGradients(gradients.mapValues { gradients[overrideMap[it.key]] ?: it.value })
    }
}
