...
Для настройки стиля LinkButton в формате xml предусмотрены следующие атрибуты:
...
Для задания лэйбла (основного текста внутри LinkButton), а так же конфигурирования стиля и цвета текста - используйте  
...
```xml
<!-- @sample: com/sdds/uikit/fixtures/LinkButtonStyle.xml -->
```

## Расположение контента внутри LinkButton
...
```kotlin
// @sample: com/sdds/uikit/fixtures/samples/button/LinkButton_LabelValue_Packed.kt
```

или SpaceBetween (Контент отобразится по всей ширине, отступы будут добавлены между элементами контента):
```kotlin
// @sample: com/sdds/uikit/fixtures/samples/button/LinkButton_LabelValue_SpaceBetween.kt
```
