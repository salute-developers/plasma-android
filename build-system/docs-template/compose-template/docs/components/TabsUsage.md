...
Элемент `Tabs`. При использовании внутри `Tabs` компонент стилизуется автоматически. Если нужен отдельный стиль для элемента, используйте `TabItemStyle.builder()`. Общие принципы работы `Style` и `StyleBuilder` описаны в разделе [Стилизация компонентов](../theme/Styles.md).
