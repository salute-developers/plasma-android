# docs-template-consumer-guides Specification

## Purpose
TBD - created by archiving change add-mcp-cli-docs-template. Update Purpose after archive.
## Requirements
...
- **THEN** страница SHALL описывать, что запуск MCP-сервера предполагает установленный CLI (установка — на странице `cli.md`)
- **THEN** страница SHALL содержать пример конфигурации `mcpServers` для MCP-клиента с `command: "dsbuilder"` и `args: ["mcp", "serve"]`
- **THEN** страница SHALL явно указывать, что MCP-клиент сам запускает `dsbuilder mcp serve` по этой конфигурации и что команду не нужно запускать отдельно вручную в терминале
- **THEN** `mcp.md` SHALL NOT содержать отдельный шаг с самостоятельным bash-блоком `dsbuilder mcp serve`, инструктирующий читателя запустить сервер вручную до настройки клиента
