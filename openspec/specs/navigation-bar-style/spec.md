# navigation-bar-style Specification

## Purpose
API стиля `NavigationBar` и `CollapsingNavigationBar` в части минимальной высоты контент-области: свойство `minContentHeight` в `NavigationBarDimensions` и `CollapsingNavigationBarDimensions` (Compose) и атрибут `sd_minContentHeight` (View).
