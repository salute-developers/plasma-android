...
- [x] 1.1 Создать `mcp.md` с front matter (`id`, `title`, `sidebar_position`), согласованным со стилем существующих страниц (`quick_start.md`, `theme/*.md`). **Ревью-фидбэк**: `title` заменён с «MCP» на «Агентская разработка с DS Builder MCP»
...
- [x] 1.3.2 **Ревью-фидбэк**: убрать отдельный шаг «Запустите MCP-сервер: `dsbuilder mcp serve`» — команду не запускают вручную, её вызывает сам MCP-клиент по конфигурации `mcpServers`. Осталось 2 шага (установить/настроить CLI → добавить в конфиг клиента), пояснение «клиент сам запускает `dsbuilder mcp serve`, запускать отдельно не нужно» перенесено под JSON-конфиг
...
- [x] 2.1 Создать `cli.md` с front matter, согласованным со стилем существующих страниц. **Ревью-фидбэк**: `title` заменён с «CLI» на «Использование DS Builder CLI»
...
- [x] 2.7 Короткий раздел «MCP-сервер» (`dsbuilder mcp serve`) с одной ссылкой-мостиком на `mcp.md` за конфигурацией клиента и списком инструментов, без дублирования этого списка. **Ревью-фидбэк**: убран отдельный bash-блок с командой, текст поясняет, что `dsbuilder mcp serve` обычно вызывает сам MCP-клиент, а не запускают вручную — симметрично правке в `mcp.md`
