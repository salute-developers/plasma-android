...
- **WHEN** CI собирает модуль `icons-compose` с несколькими тысячами сгенерированных файлов
- **THEN** heap настроен так (`org.gradle.jvmargs` / `kotlin.daemon.jvmargs`), что компиляция завершается без OutOfMemoryError
