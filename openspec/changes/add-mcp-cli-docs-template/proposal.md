...
- `mcp.md` описывает только read-only consumer-сценарий: быстрый старт через `npx dsbuilder-mcp serve` без установки CLI, пример конфига `mcpServers`, explicit `designSystem`-ссылку для передачи контекста без локального `.sdds/config.json`, и краткий список read-only MCP tools.
- `cli.md` описывает только consumer-сценарий: установку `dsbuilder` из GitHub Release, `dsbuilder init`/`auth login`/`status`, и `dsbuilder mcp serve` как встроенную команду CLI (с отсылкой к `mcp.md` за конфигом клиента). Write/maintainer-команды (`theme fetch/push`, `components push`, `docs publish`) явно исключены из страницы.
...
Важное допущение: `npx dsbuilder-mcp` и релиз `dsbuilder-cli-macos-arm64.zip` на GitHub Releases `design-system-builder` на момент написания не существуют — там пока только локальные Gradle-таски (`packageMcpNodeNpm`, `frontend-kt/cli/build/distributions/...`) без release/publish workflow. Инструкция сознательно описывает целевое, ещё не опубликованное поведение — она должна synchронизироваться с моментом появления реального релизного пайплайна в `design-system-builder`.
